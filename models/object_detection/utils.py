...
def remap_class_ids(detections: sv.Detections, ordered_class_names: List[str]) -> None:
    ...
    class_ids = [ordered_class_names.index(class_name) for class_name in detections.data["class_name"]]
    detections.class_id = np.array(class_ids)


def write_json_results(model_name: str, model: nn.Module, mAP_result: MeanAveragePrecisionResult) -> None:
    result: dict[str, Any] = {}
